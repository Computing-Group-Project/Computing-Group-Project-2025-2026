...
const QueueItem = ({ order, onCancel, onAccept, onStartPreparing, onMarkReady, onMarkCompleted }) => {
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm mb-3 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors duration-200">
      ...
      <div className="flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0">
        ...
        {status === 'Confirmed' && (
          <button onClick={onStartPreparing} aria-label="Start preparing order" className="flex-1 md:flex-none px-6 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold rounded-lg shadow-sm transition-colors">
            Start Preparing
          </button>
        )}
